      ...
      for (int c = 0; c < C; ++c) {
        ...
        for (int ky = -1; ky <= 1; ++ky) {
          ...
          for (int kx = -1; kx <= 1; ++kx) {
            ...
            // 특정 픽셀의 특정 채널 값과 그에 매칭 되는 필터의 가중치 값을
            // 역양자화한 뒤 곱하고 누적.
          }
        }
        ...
        if (apply_relu6) {
          ...
          if (q > q6)
            q = (q6 > 255 ? 255 : (q6 < 0 ? 0 : (uint8_t)q6));
          if (q < out->zp)
            q = (uint8_t)out->zp;
        }
...
void pwconv1x1_nhwc_u8(const tensor_u8_nhwc_t *in, const uint8_t *k1x1,
                       const int32_t *bias, float w_scale, int w_zp,
                       tensor_u8_nhwc_t *out) {
